...
// ── Top stat card ─────────────────────────────────────────────
const TopStatCard = ({ label, value, icon, color, trendValue, loading }) => {
...
// ── Info mini card ────────────────────────────────────────────
const InfoMiniCard = ({ label, value, sub, icon, bg, loading }) => (
  <Card sx={{ background: bg, boxShadow: 'none', border: `1px solid ${KUKAT.border}` }}>
    <CardContent sx={{ p: '16px !important' }}>
      <Box sx={{ mb: 1 }}>{icon}</Box>
      {loading
        ? <Skeleton width={60} height={28} />
        : <Typography sx={{ fontSize: '1.4rem', fontWeight: 700, color: KUKAT.navy, lineHeight: 1 }}>
            {value}
          </Typography>
      }
      <Typography variant="caption" sx={{ color: KUKAT.navy, fontWeight: 600, display: 'block', mt: 0.3 }}>
        {label}
      </Typography>
      <Typography variant="caption" sx={{ color: KUKAT.textMuted, fontSize: '0.7rem' }}>
        {sub}
...
const AdminDashboard = ({
  bk, inv, comm, cust, trend, agents, destinations,
  statusPie, agentBarData, loading, rLoading,
  navigate, maxDest, recentBookings, recentPayments,
}) => (
  <>
    {/* ROW 1: Stat cards */}
    <Box sx={{
      ...
      {/* ← "Commission income" — money received FROM suppliers */}
      <TopStatCard label="Commission income" value={fmtK(comm.totalPaid)}
        icon={<AccountBalance />} color={CHART_COLORS.purple}
        trendValue={comm.overdueCount > 0 ? -5 : 5} loading={loading} />
    </Box>

    {/* ROW 2: Mini chart cards */}
    <Box sx={{
      display: 'grid',
      ...
      <MiniChartCard title="Total revenue"
        ...
        icon={<AttachMoney />} data={trend} dataKey="revenue"
    ...
    <Card sx={{ mb: 2.5 }}>
      <CardHeader title="Agent performance breakdown"
        titleTypographyProps={{ variant: 'h6', sx: { color: KUKAT.navy, fontSize: '1rem' } }}
        subheader="Bookings, revenue and commission income generated per agent"
        ...
        {rLoading ? <Skeleton variant="rounded" height={200} /> :
         agents.length === 0 ? (
          ...
          <Table size="small">
            <TableHead>
              <TableRow>
                <TableCell sx={{ fontWeight: 700, background: KUKAT.navy, color: '#fff' }}>AGENT</TableCell>
                <TableCell align="center" sx={{ fontWeight: 700, background: KUKAT.navy, color: '#fff' }}>BOOKINGS</TableCell>
                <TableCell align="center" sx={{ fontWeight: 700, background: KUKAT.navy, color: '#fff' }}>REVENUE</TableCell>
                {/* ← Renamed from COMMISSIONS to COMMISSION GENERATED */}
                <TableCell align="center" sx={{ fontWeight: 700, background: KUKAT.navy, color: '#fff' }}>COMMISSION GENERATED</TableCell>
                <TableCell align="center" sx={{ fontWeight: 700, background: KUKAT.navy, color: '#fff' }}>CONVERSION</TableCell>
              </TableRow>
            </TableHead>
            <TableBody>
              {agents.map((a, i) => {
                const convRate = a.bookingCount > 0
                  ? Math.round((a.bookingCount / (a.bookingCount + (a.lostCount || 0))) * 100)
                  : 0;
                return (
                  <TableRow key={a.employeeID} sx={{ background: i % 2 === 0 ? '#fff' : KUKAT.surface }}>
                    <TableCell>
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                        ...
                        <Box>
                          <Typography variant="body2" fontWeight={600}>{a.firstName} {a.lastName}</Typography>
                    ...
                    <TableCell align="center">
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                        ...
                        <Typography variant="caption" fontWeight={700}
                          sx={{ color: KUKAT.teal, minWidth: 30 }}>
                          {convRate}%
                        </Typography>
                      </Box>
                    </TableCell>
    ...
    {/* ROW 5: Donut chart + bar chart */}
    <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: '4fr 8fr' }, gap: 2, mb: 3 }}>
      <Card>
      ...
      <Card>
        <CardHeader title="Agent revenue vs commission generated"
          titleTypographyProps={{ variant: 'h6', sx: { color: KUKAT.navy, fontSize: '1rem' } }} />
        <CardContent sx={{ pt: 0 }}>
            ...
            <Box sx={{ height: 280 }}>
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={agentBarData} margin={{ top: 5, right: 20, left: 10, bottom: 5 }}>
                  ...
                  <Bar dataKey="commissionGenerated" name="Commission generated" fill={CHART_COLORS.amber}  radius={[4,4,0,0]} />
                </BarChart>
              </ResponsiveContainer>
            </Box>
          )}
      ...
      <Card sx={{ background: KUKAT.navy }}>
        <CardContent sx={{ p: 3, height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
          <Box>
            ...
            <Typography sx={{ color: '#fff', fontSize: '1.3rem', fontWeight: 700, mt: 1.5, mb: 1.5, lineHeight: 1.3 }}>
              Your agency performance at a glance
            </Typography>
            <Typography variant="body2" sx={{ color: 'rgba(255,255,255,0.6)', lineHeight: 1.7 }}>
              Track bookings, revenue, supplier commissions and agent performance across all time periods.
            </Typography>
          </Box>
          <Box sx={{ mt: 3 }}>
            <Chip label="View reports →" onClick={() => navigate('/reports')}
              sx={{ background: KUKAT.amber, color: KUKAT.navy, fontWeight: 700,
                cursor: 'pointer', fontSize: '0.8rem', '&:hover': { background: KUKAT.amberLight } }} />
          </Box>
        </CardContent>
      </Card>

      {/* ← Replaced Grid item with CSS Grid box */}
      <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 2 }}>
        {[
          { label: 'Total invoices',       value: fmtShort(inv.total),          sub: 'All time',
            icon: <Receipt sx={{ fontSize: 28, color: KUKAT.teal }} />,          bg: `${KUKAT.teal}10` },
          { label: 'Collected',            value: fmtK(inv.totalCollected),      sub: 'Paid invoices',
            icon: <AttachMoney sx={{ fontSize: 28, color: '#15803D' }} />,        bg: '#DCFCE7' },
          // ← "Pending from suppliers" — commission direction updated label
          { label: 'Commissions pending',  value: fmtShort(comm.pending),        sub: 'Pending from suppliers',
            icon: <HourglassEmpty sx={{ fontSize: 28, color: CHART_COLORS.amber }} />, bg: '#FEF9C3' },
          { label: 'Top destination',
            value: destinations[0]?.destinationName?.split(' ')[0] ?? '—',
            sub: `${destinations[0]?.bookingCount ?? 0} bookings`,
            icon: <FlightTakeoff sx={{ fontSize: 28, color: CHART_COLORS.purple }} />, bg: `${CHART_COLORS.purple}10` },
        ].map((item, i) => (
          <InfoMiniCard key={i} {...item} loading={loading} />
        ))}
      </Box>
    </Box>
        ...
        <CardHeader title="Recent bookings"
          ...
          action={
            <Typography variant="body2" onClick={() => navigate('/bookings')}
              sx={{ color: KUKAT.teal, cursor: 'pointer', mt: 0.5, '&:hover': { textDecoration: 'underline' } }}>
              View all →
            </Typography>
          }
        />
              ...
              <TableBody>
                {(recentBookings ?? []).map((b, i) => (
                  <TableRow key={b.bookingID}
                    ...
                    <TableCell><Typography variant="body2">{b.productName}</Typography></TableCell>
            ...
            (recentPayments ?? []).map((p) => (
              <Box key={p.paymentID} onClick={() => navigate(`/invoices/${p.invoiceID}`)}
                sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between',
                  py: 1.5, cursor: 'pointer', borderBottom: `1px solid ${KUKAT.border}`,
                  '&:last-child': { borderBottom: 'none' }, '&:hover': { background: KUKAT.surface } }}>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
...
const HRDashboard = ({ data, loading }) => {
  ...
  const rolePieData        = roleDistribution.map(r => ({ name: r.roleName, value: r.count }));

  return (
    <Box sx={{ width: '100%', maxWidth: '1600px', mx: 'auto' }}>
      {/* ROW 1: Stat cards */}
      <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: 'repeat(4, 1fr)' }, gap: 1, mb: 3 }}>
        <TopStatCard label="Total employees"      value={fmtShort(employeeStats.totalEmployees)}
          icon={<People />}        color={KUKAT.navy}           trendValue={employeeStats.newHiresThisYear > 0 ? 8 : 0} loading={loading} />
        <TopStatCard label="Active employees"     value={fmtShort(employeeStats.activeEmployees)}
          icon={<CheckCircle />}   color={KUKAT.teal}           trendValue={15} loading={loading} />
        <TopStatCard label="New hires (this year)" value={fmtShort(employeeStats.newHiresThisYear)}
          icon={<TrendingUp />}    color={CHART_COLORS.amber}   trendValue={12} loading={loading} />
        <TopStatCard label="Avg bookings/agent"   value={fmtShort(agentMetrics.avgBookingsPerAgent)}
          icon={<AccountBalance />} color={CHART_COLORS.purple} trendValue={-5} loading={loading} />
      </Box>

      {/* ROW 2: Charts */}
      <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: 'repeat(2, 1fr)' }, gap: 2, mb: 3 }}>
        <Card>
          <CardHeader title="Employee role distribution"
            titleTypographyProps={{ variant: 'h6', sx: { color: KUKAT.navy, fontSize: '1rem' } }} />
          <CardContent sx={{ pt: 0 }}>
        ...
        <Card>
          <CardHeader title="Top performing employees"
            titleTypographyProps={{ variant: 'h6', sx: { color: KUKAT.navy, fontSize: '1rem' } }} />
          <CardContent sx={{ pt: 0 }}>
            {loading ? <Skeleton variant="rounded" height={300} /> :
             employeePerformance.length === 0 ? (
              ...
              <Table size="small">
                <TableHead>
                  <TableRow>
                    <TableCell sx={{ fontWeight: 700, background: KUKAT.navy, color: '#fff' }}>EMPLOYEE</TableCell>
                    <TableCell align="center" sx={{ fontWeight: 700, background: KUKAT.navy, color: '#fff' }}>BOOKINGS</TableCell>
                    <TableCell align="center" sx={{ fontWeight: 700, background: KUKAT.navy, color: '#fff' }}>REVENUE</TableCell>
                  </TableRow>
                </TableHead>
                <TableBody>
                  {employeePerformance.slice(0, 8).map((emp, i) => (
                    <TableRow key={emp.employeeID} sx={{ background: i % 2 === 0 ? '#fff' : KUKAT.surface }}>
                      <TableCell>
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                          <Avatar sx={{ width: 28, height: 28, fontSize: '0.7rem', fontWeight: 700,
                            background: `${KUKAT.navy}15`, color: KUKAT.navy }}>
                          ...
                          <Box>
                            ...
                            <Typography variant="caption" sx={{ color: KUKAT.textMuted }}>{emp.agentCode}</Typography>
                          </Box>
                        </Box>
                      </TableCell>
                      <TableCell align="center">
                        <Typography variant="body2" fontWeight={600}>{emp.bookingCount?.toLocaleString() ?? 0}</Typography>
                      </TableCell>
                      <TableCell align="center">
                        <Typography variant="body2" fontWeight={600} sx={{ color: KUKAT.teal }}>{fmt(emp.totalRevenue)}</Typography>
                      </TableCell>
                    </TableRow>
                  ))}
                </TableBody>
      ...
      <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: '4fr 8fr' }, gap: 2, mb: 3 }}>
        <Card sx={{ background: KUKAT.navy }}>
          <CardContent sx={{ p: 3, display: 'flex', flexDirection: 'column', justifyContent: 'space-between', height: '100%' }}>
            ...
            <Box sx={{ mt: 3 }}>
              <Chip label="View employees →" onClick={() => navigate('/staff')}
                sx={{ background: KUKAT.amber, color: KUKAT.navy, fontWeight: 700,
                  cursor: 'pointer', fontSize: '0.8rem', '&:hover': { background: KUKAT.amberLight } }} />
            </Box>
          </CardContent>
        ...
        {/* ← Replaced Grid item with CSS Grid */}
        <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 2 }}>
          {[
            { label: 'Active employees', value: fmtShort(employeeStats.activeEmployees),
              sub: `${employeeStats.totalEmployees} total`,
              icon: <People sx={{ fontSize: 28, color: KUKAT.teal }} />, bg: `${KUKAT.teal}10` },
            { label: 'Role types', value: fmtShort(roleDistribution.length),
              sub: 'Different positions',
              icon: <AccountBalance sx={{ fontSize: 28, color: CHART_COLORS.amber }} />, bg: '#FEF9C3' },
            { label: 'Top performer', value: employeePerformance[0]?.bookingCount ?? 0,
              sub: 'Bookings this period',
              icon: <TrendingUp sx={{ fontSize: 28, color: '#15803D' }} />, bg: '#DCFCE7' },
            { label: 'Avg performance', value: fmtShort(agentMetrics.avgBookingsPerAgent),
              sub: 'Bookings per agent',
              icon: <CheckCircle sx={{ fontSize: 28, color: CHART_COLORS.purple }} />, bg: `${CHART_COLORS.purple}10` },
          ].map((item, i) => <InfoMiniCard key={i} {...item} loading={loading} />)}
        </Box>
      </Box>
    </Box>
  );
    ...
    <Box sx={{ width: '100%', maxWidth: '1600px', mx: 'auto' }}>
      ...
      <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: 'repeat(4, 1fr)' }, gap: 2, mb: 3 }}>
        <TopStatCard label="Total collected"    value={fmtK(financialOverview.totalCollected)}
          icon={<AttachMoney />}   color={KUKAT.teal}           trendValue={16} loading={loading} />
        <TopStatCard label="Outstanding"        value={fmtK(financialOverview.totalOutstanding)}
          icon={<HourglassEmpty />} color={CHART_COLORS.red}    trendValue={-8} loading={loading} />
        {/* ← "Commission income received" — money received from suppliers */}
        <TopStatCard label="Commission income received" value={fmtK(commissionPayments.totalPaidCommissions)}
          icon={<AccountBalance />} color={CHART_COLORS.purple} trendValue={12} loading={loading} />
        <TopStatCard label="Avg commission rate"
          value={`${(parseFloat(commissionPayments.avgCommissionRate || 0) * 100).toFixed(1)}%`}
          icon={<TrendingUp />}    color={CHART_COLORS.amber}   trendValue={5}  loading={loading} />
      </Box>
      ...
      <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: '7fr 5fr' }, gap: 2, mb: 3 }}>
              ...
              <Box sx={{ height: 300 }}>
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={revenueChartData} margin={{ top: 5, right: 20, left: 10, bottom: 5 }}>
                    ...
                    <YAxis tick={{ fontSize: 11, fill: KUKAT.textMuted }} tickLine={false} axisLine={false} tickFormatter={fmtK} width={55} />
                    <Tooltip content={<CustomTooltip isCurrency />} />
                    <defs>
                    ...
                    <Area type="monotone" dataKey="revenue" stroke={KUKAT.teal} strokeWidth={2.5}
        ...
        <Card>
          <CardHeader title="Payment methods breakdown"
            titleTypographyProps={{ variant: 'h6', sx: { color: KUKAT.navy, fontSize: '1rem' } }} />
          <CardContent sx={{ pt: 0 }}>
            {loading ? <Skeleton variant="rounded" height={300} /> :
          ...
          {loading ? <Skeleton variant="rounded" height={200} /> :
           outstandingInvoices.length === 0 ? (
            ...
            <Table size="small">
              <TableHead>
                <TableRow>
                  <TableCell sx={{ fontWeight: 700, background: KUKAT.navy, color: '#fff' }}>CUSTOMER</TableCell>
                  <TableCell align="center" sx={{ fontWeight: 700, background: KUKAT.navy, color: '#fff' }}>AMOUNT</TableCell>
                  <TableCell align="center" sx={{ fontWeight: 700, background: KUKAT.navy, color: '#fff' }}>STATUS</TableCell>
                  <TableCell align="center" sx={{ fontWeight: 700, background: KUKAT.navy, color: '#fff' }}>DAYS OUTSTANDING</TableCell>
                </TableRow>
              </TableHead>
              <TableBody>
                {outstandingInvoices.map((inv, i) => (
                  <TableRow key={inv.invoiceID}
                    ...
                    <TableCell><Typography variant="body2" fontWeight={600}>{inv.customerName}</Typography></TableCell>
                    <TableCell align="center">
                      <Typography variant="body2" fontWeight={600} sx={{ color: CHART_COLORS.red }}>
                        {fmt(inv.totalAmount)}
                      </Typography>
                    </TableCell>
                    <TableCell align="center"><StatusChip status={inv.status} /></TableCell>
                    <TableCell align="center">
                      <Typography variant="body2" fontWeight={600}>{inv.daysOutstanding} days</Typography>
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>
      ...
      <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: '7fr 5fr' }, gap: 2, mb: 3 }}>
        <Card sx={{ background: KUKAT.navy }}>
          <CardContent sx={{ p: 3, display: 'flex', flexDirection: 'column', justifyContent: 'space-between', height: '100%' }}>
            <Box>
              ...
              <Typography variant="body2" sx={{ color: 'rgba(255,255,255,0.6)', lineHeight: 1.7 }}>
                Monitor financial performance, payment methods, supplier commission receipts and outstanding invoices.
              </Typography>
            </Box>
            <Box sx={{ mt: 3 }}>
              <Chip label="View invoices →" onClick={() => navigate('/invoices')}
                sx={{ background: KUKAT.amber, color: KUKAT.navy, fontWeight: 700,
                  cursor: 'pointer', fontSize: '0.8rem', '&:hover': { background: KUKAT.amberLight } }} />
            </Box>
          </CardContent>
        ...
        <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 2 }}>
          {[
            { label: 'Total invoices',    value: fmtShort(financialOverview.totalInvoices), sub: 'All time',
              icon: <Receipt sx={{ fontSize: 28, color: KUKAT.teal }} />,           bg: `${KUKAT.teal}10` },
            { label: 'Payment methods',   value: fmtShort(paymentMethods.length),  sub: 'Active methods',
              icon: <AttachMoney sx={{ fontSize: 28, color: '#15803D' }} />,         bg: '#DCFCE7' },
            // ← "Awaiting from suppliers" — commission direction
            { label: 'Commissions pending', value: fmtK(commissionPayments.pendingCommissions), sub: 'Awaiting from suppliers',
              icon: <HourglassEmpty sx={{ fontSize: 28, color: CHART_COLORS.amber }} />, bg: '#FEF9C3' },
            { label: 'Outstanding amount', value: fmtK(financialOverview.totalOutstanding), sub: 'Needs collection',
              icon: <AccountBalance sx={{ fontSize: 28, color: CHART_COLORS.red }} />, bg: `${CHART_COLORS.red}10` },
          ].map((item, i) => <InfoMiniCard key={i} {...item} loading={loading} />)}
        </Box>
      </Box>
    </Box>
...
const AgentDashboard = ({ data, loading }) => {
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2.5 }}>
      ...
      {/* ROW 1: 3 stat cards — commissions removed for agents */}
      <Box sx={{
        display: 'grid',
        gridTemplateColumns: { xs: '1fr 1fr', md: 'repeat(3, 1fr)' }, // ← 3 not 4
        gap: 2,
      }}>
      ...
      <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: '1fr 1fr' }, gap: 2, alignItems: 'start' }}>
          ...
          <CardContent sx={{ pt: 0 }}>
            <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 2 }}>
              {[
                ...
                <Box key={s.label} sx={{ p: 2, borderRadius: 2, background: s.bg, textAlign: 'center' }}>
                  {loading
                  ...
                  <Typography variant="caption" sx={{ color: s.color, fontWeight: 500 }}>{s.label}</Typography>
                </Box>
              ))}
            </Box>
          </CardContent>
        ...
        {/* ← Invoice summary replaces commission card */}
        <Card>
          <CardHeader title="My invoices"
            titleTypographyProps={{ variant: 'h6', sx: { color: KUKAT.navy, fontSize: '1rem' } }} />
          <CardContent sx={{ pt: 0 }}>
            {[
              { label: 'Total invoices', value: fmtShort(invoices.total),        color: KUKAT.navy },
              { label: 'Paid',           value: fmtShort(invoices.paid),         color: '#15803D' },
              { label: 'Unpaid',         value: fmtShort(invoices.unpaid),       color: '#DC2626' },
              { label: 'Revenue collected', value: fmtK(invoices.totalCollected), color: KUKAT.teal },
            ].map((item, i) => (
              <Box key={i} sx={{ display: 'flex', justifyContent: 'space-between',
                alignItems: 'center', py: 1.5,
                borderBottom: i < 3 ? `1px solid ${KUKAT.border}` : 'none' }}>
                <Typography variant="body2" sx={{ color: KUKAT.textMuted }}>{item.label}</Typography>
                {loading
            ...
            <Button size="small" variant="outlined" fullWidth
              onClick={() => navigate('/invoices')} sx={{ mt: 2 }}>
              View all invoices →
            </Button>
          </CardContent>
        </Card>
      ...
      <Card>
        <CardHeader title="My recent bookings"
          titleTypographyProps={{ variant: 'h6', sx: { color: KUKAT.navy, fontSize: '1rem' } }}
          action={
            <Typography variant="body2" onClick={() => navigate('/bookings')}
      ...
      <Card sx={{ background: KUKAT.navy }}>
        <CardContent sx={{ p: 3, display: 'flex', flexDirection: 'column', gap: 1.5 }}>
          ...
          <Typography variant="body2" sx={{ color: 'rgba(255,255,255,0.6)', lineHeight: 1.7 }}>
            Track your bookings, customers and invoice collections.
            Contact your manager for performance reviews and targets.
          </Typography>
          <Box sx={{ display: 'flex', gap: 1.5, mt: 1 }}>
            <Chip label="New booking →" onClick={() => navigate('/bookings')}
              sx={{ background: KUKAT.amber, color: KUKAT.navy, fontWeight: 700,
                cursor: 'pointer', fontSize: '0.8rem', '&:hover': { background: KUKAT.amberLight } }} />
            <Chip label="My customers →" onClick={() => navigate('/customers')}
              sx={{ background: 'rgba(255,255,255,0.15)', color: '#fff', fontWeight: 600,
                cursor: 'pointer', fontSize: '0.8rem', '&:hover': { background: 'rgba(255,255,255,0.25)' } }} />
          </Box>
        </CardContent>
      </Card>
    </Box>
...
export default function DashboardPage() {
  ...
  let bk, inv, comm, cust, trend, agents, destinations, statusPie, agentBarData;
  ...
  if (['superadmin', 'manager'].includes(userRole)) {
    ...
    // ← commissionGenerated matches reportsController field name
    agentBarData = agents.slice(0, 6).map(a => ({
      name:                a.firstName,
      bookings:            a.bookingCount || 0,
      revenue:             parseFloat(a.revenue              || 0),
      commissionGenerated: parseFloat(a.commissionGenerated  || 0),
    }));
  }
  ...
  return (
    <AppLayout title="Dashboard" subtitle={`Good day, ${user?.firstName}`}>
      <Box sx={{ width: '100%', maxWidth: '1600px', mx: 'auto' }}>

        {error && <Alert severity="error" sx={{ mb: 2 }}>{error}</Alert>}

        {/* Period toggle — superadmin/manager only */}
            ...
            <ToggleButtonGroup value={period} exclusive size="small"
              ...
              sx={{
                '& .MuiToggleButton-root': {
                  ...
                  '&.Mui-selected': {
                    background: KUKAT.navy, color: '#fff', borderColor: KUKAT.navy,
