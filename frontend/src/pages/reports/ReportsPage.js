...
import {
  ...
  TextField, MenuItem, Button, Alert, Skeleton, Divider,
      ...
      <Grid container spacing={2} sx={{ mb: 3 }}>
        <Grid item xs={6} sm={3}>
          <KPICard label="Total revenue" loading={loading} color={KUKAT.teal}
            value={rev.total != null ? `$${parseFloat(rev.total).toLocaleString('en-CA', { minimumFractionDigits: 0 })}` : '—'}
            sub="From paid invoices" />
        </Grid>
        <Grid item xs={6} sm={3}>
          <KPICard label="Total bookings" loading={loading} color={KUKAT.navy}
            value={bk.total ?? '—'} sub={`${bk.confirmed ?? 0} confirmed`} />
        </Grid>
        <Grid item xs={6} sm={3}>
          <KPICard label="Commissions paid" loading={loading} color="#15803D"
            value={comm.paid != null ? `$${parseFloat(comm.paid).toLocaleString('en-CA', { minimumFractionDigits: 0 })}` : '—'}
            sub={`${comm.pendingCount ?? 0} pending approval`} />
        </Grid>
        <Grid item xs={6} sm={3}>
          <KPICard label="Avg booking value" loading={loading} color={KUKAT.amber}
            value={rev.avgBooking != null ? `$${parseFloat(rev.avgBooking).toFixed(0)}` : '—'}
        ...
        <Grid item xs={12} md={6}>
          <Card sx={{ height: '100%' }}>
            ...
            <CardContent sx={{ pt: 0 }}>
              ...
              agents.length === 0 ? (
                <Typography variant="body2" sx={{ color: KUKAT.textMuted, py: 3, textAlign: 'center' }}>No data.</Typography>
              ) : agents.map((a) => (
                <Box key={a.employeeID} sx={{ mb: 2.5 }}>
                  ...
                  <Box sx={{ display: 'flex', gap: 3, pl: 4.5 }}>
                    <Typography variant="caption" sx={{ color: KUKAT.textMuted }}>
                      Revenue: <strong style={{ color: KUKAT.navy }}>${parseFloat(a.revenue || 0).toLocaleString('en-CA', { minimumFractionDigits: 0 })}</strong>
                    </Typography>
                    <Typography variant="caption" sx={{ color: KUKAT.textMuted }}>
                      Commission: <strong style={{ color: '#15803D' }}>${parseFloat(a.commission || 0).toFixed(0)}</strong>
                    </Typography>
                  </Box>
                </Box>
              ))}
                      ...
                      <TableRow key={p.productID}>
                        <TableCell>
                          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                            <Typography sx={{ fontSize: '0.7rem', fontWeight: 700, color: KUKAT.textMuted,
                        ...
                        <TableCell>
                          <Typography variant="caption" sx={{ color: KUKAT.textMuted }}>{p.supplierName}</Typography>
                        </TableCell>
                ...
                <Grid container spacing={2}>
                  {[
                    ...
                    <Grid item xs={6} sm={3} key={s.label}>
                      <Box sx={{ p: 2, borderRadius: 2, background: s.bg, textAlign: 'center' }}>
